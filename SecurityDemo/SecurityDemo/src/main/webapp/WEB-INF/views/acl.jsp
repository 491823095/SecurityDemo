...
<script type="text/javascript">
    $(function () {
            ...
            $.ajax({
                ...
                success:function (result) {
                    if (result.ret){
                        ...
                        var renderd=Mustache.render(aclModuleListTemplate,{
                            aclModuleList:result.data,
            ...
            $(".aclModule-delete").click(function (e) {
                ...
                if (confirm("确定要删除权限模块["+aclModuleName+"]吗?")){
                    $.ajax({
                        ...
                        success:function (result) {
                            if (result.ret){
                                showMessage("删除权限模块模块["+aclModuleName+"]","操作成功,true");
                                loadAclModuleTree();
                            }else{
                                showMessage("删除权限模块模块["+aclModuleName+"]",result.msg,false);
                            }
                        }
                    })
                }
        ...
        function handleAclModuleSelected(aclModuleId) {
            ...
            //通过权限模块id查询并显示旗下所有权限模块方法
            loadAclList(aclModuleId);
        }  //handleAclModuleSelected
